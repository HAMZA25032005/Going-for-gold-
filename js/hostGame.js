...
function updateTimer(){
    time = 10;
    var countdownAudioTenSeconds = new Audio('/audios/10 seconds.mp3'); // Charger l'audio 10 seconds.mp3
    countdownAudioTenSeconds.play(); // Lancer l'audio
    ...
    timer = setInterval(function(){
        ...
        if(time == 0){
            clearInterval(timer);
            countdownAudioTenSeconds.pause(); // Arrêter l'audio lorsque le décompte atteint 0
            socket.emit('timeUp');
        }
    }, 1000);
}
